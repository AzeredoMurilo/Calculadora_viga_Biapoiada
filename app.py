...
# Renderização dinâmica dos campos com base no tipo de carga
w1, w2 = None, None
posicao_f = None
...
if tipo_carga == "Triangular / Trapezoidal":
    ...
    valor = st.sidebar.number_input("Magnitude (kN ou kN.m)", value=-10.0, step=1.0, 
                                    help="Use valores negativos para baixo/horário.")
...
if len(st.session_state.cargas_salvas) > 0:
    # Mostra uma tabela limpa para o usuário
...
if st.button("🚀 Calcular Diagramas") and len(st.session_state.cargas_salvas) > 0:
    ...
    with st.spinner("Resolvendo estática e integrais (incluindo rampas)..."):
        ...
        # --- APLICANDO AS CARGAS (A MÁGICA DA ORDEM 1 ACONTECE AQUI) ---
        for carga in st.session_state.cargas_salvas:
            ...
            if tipo == "Pontual":
                ...
                w1 = carga["w1"]
                w2 = carga["w2"]
                # Taxa de variação (inclinação da reta)
                q = (w2 - w1) / (pf - pi)
                
                # 1. Aplica o valor base (ordem 0)
                if w1 != 0:
                    viga.apply_load(w1, pi, 0)
                    
                # 2. Aplica a rampa crescente/decrescente (ordem 1)
                if q != 0:
                    viga.apply_load(q, pi, 1)
                    
                # 3. Anula a carga na posição final para que não vá até o infinito
                if pf <= comprimento:
                    if w2 != 0:
                        viga.apply_load(-w2, pf, 0) # Anula o montante acumulado
                    if q != 0:
                        viga.apply_load(-q, pf, 1)  # Anula a rampa
        ...
        x_vetor = np.linspace(0, float(comprimento), 800) # Aumentei para 800 pontos para curvas mais suaves
